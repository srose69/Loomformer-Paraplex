...
__global__ void tria_init_seed_gate_forward_kernel(
    ...
    const int64_t t = bt % T;
    const int64_t b = bt / T;
    const scalar_t* wh = w + (h / hidden_per_head) * 9;
    float m[9], pre[9];
    tria_carrier_build9((float)r[idx], (float)i_[idx], (float)o[idx], alpha, axis, m);
    if (t == 0 && valid[b]) {
        float seed_vals[9];
...
__global__ void tria_init_seed_gate_backward_kernel(
    ...
    float alpha, int axis, int64_t B, int64_t T, int64_t H,
    int64_t hidden_per_head, int64_t heads, int64_t chunks_per_head,
    int64_t partials_per_head) {
    const int64_t n = B * T * H;
    int64_t idx, head, partial;
    const bool valid_idx = gate_mix_head_index(
    ...
    if (valid_idx) {
        ...
        if (use_seed) {
            ...
            #pragma unroll
            for (int k = 0; k < 9; ++k) {
                gout[k] = (float)g[k] + gp * (float)wh[k];
