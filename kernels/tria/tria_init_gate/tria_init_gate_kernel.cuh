...
__global__ void tria_init_gate_forward_kernel(
    ...
    float p=0.0f;
    #pragma unroll
    for(int k=0;k<9;++k){ const float cv=vals[k]*inv; out[k]=(scalar_t)cv; p=fmaf(cv,(float)wh[k],p); }
...
__global__ void tria_init_gate_backward_kernel(
    ...
    if(valid){
        ...
        #pragma unroll
        for(int k=0;k<9;++k){ gout[k]=(float)g[k]+gp*(float)wh[k]; local_w[k]=gp*vals[k]*inv; }
