// kernels/tria/tria_init/tria_init_kernel.cu -- standalone compilable TU
...
//   nvcc -arch=sm_61 --ptx tria_init_kernel.cu -o tria_init_kernel.ptx
...
// torch-free).
template __global__ void tria_init_forward_kernel<float>(const float* __restrict__ r, const float* __restrict__ i_, const float* __restrict__ o,
    float* __restrict__ carry_1,
    float* __restrict__ scale_out,
    float alpha, int axis, int64_t n);
template __global__ void tria_init_backward_kernel<float>(const float* __restrict__ grad_carry_1,
